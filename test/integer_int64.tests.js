...
describe('integer - int64', function() {
  ...
  it('should validate with minimum', function() {
    var value = 1;
    var ret = validate(helper.makeNumberParam('integer', false, 'int64', '0'), value);
    helper.validateSuccess(ret, 1, [value]);
  });
  ...
  it('should not validate with required field null', function() {
    var value = null;
    var ret = validate(helper.makeNumberParam('integer', true, 'int64'), value);
    helper.validateError(ret, 1, ["testParam is required"]);
  });

  it('should not validate with required field undefined', function() {
    var ret = validate(helper.makeNumberParam('integer', true, 'int64'), undefined);
    helper.validateError(ret, 1, ["testParam is required"]);
  });
