 ...
 * @param {Object} req The object that this is validating
 ...
 */
var object = function(param, req, models, validationObj) {
	var value = _.has(req, param.name) ? req[param.name] : undefined;
	...
	if (!_.some(ret, function(val) { return val.hasOwnProperty('error'); })) {
		if (validationObj.replaceValues) {
			req[param.name] = helper.getValue(param, ret);
		}
	}
	return ret;
};
